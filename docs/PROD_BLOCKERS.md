...
|---|---|---|---|---|---|---|---|---|
| BETA-BLK-001 | Scope | Scope v1 i lista wspieranych brokerów nie są zamrożone w jednym artefakcie | Bez tego zespół nie odróżni launch scope od backlogu i będzie dokładał przypadkowe rzeczy | Product Owner | IN PROGRESS | `docs/SCOPE_V1.md` DRAFT istnieje (6 brokerów, flow, limity, out-of-scope); czeka na potwierdzenie PO | Istnieje zaakceptowana lista: brokerzy, flow, limity i to, czego nie wspieramy w v1 | PO potwierdza lub amendent `docs/SCOPE_V1.md`, zmiana statusu na CLOSED |
| BETA-BLK-002 | Release Discipline | Brak potwierdzonej strategii `release branch/tag -> CI -> deploy` na właściwej linii releasowej | Zielony `main` nie jest tym samym co zielony kandydat do releasu | Tech Lead | READY FOR VERIFY | `docs/RELEASE_STRATEGY.md` istnieje; `.github/workflows/release.yml` triggery bezpośrednio na `push: tags: ['v[0-9]*']` (nie workflow_run — unika head_branch ambiguity dla tagów); 3 etapy CI + deploy w jednym workflow; RC tag jeszcze nie wypchnięty | Jest release branch/tag policy, a CI i deploy są sprawdzone na tagu RC | Wypchnąć `v0.1.0-rc.1`, obserwować release.yml w GitHub Actions, dodać link do run + smoke test evidence tu |
...
| BETA-BLK-005 | Correctness | PIT-38 XML nie jest walidowany oficjalnym XSD MF | Obecne minimum strukturalne nie daje wystarczającej pewności eksportu | QA Lead | CLOSED | `tests/Fixtures/official_xsd/schemat.xsd` zbundlowany lokalnie (z dependencjami); `PIT38OfficialXsdValidationTest` (5 scenariuszy) przechodzi w CI; generator naprawiony: P_26/P_27/P_28/P_31/P_32/P_33/P_51 zawsze emitowane, P_43 tylko w grupie z P_41/P_42, pola integer bez decimali, DataUrodzenia dodane do DTO | CI waliduje XML na oficjalnym XSD MF | ZAMKNIĘTE |
...
| BETA-BLK-008 | Observability | Monitoring i alerting są opisane, ale brak dowodu działania w środowisku docelowym | Bez tego pierwsze incydenty wyjdą od użytkowników | DevOps | IN PROGRESS | `/health` controller zaimplementowany (`src/Shared/Infrastructure/Controller/HealthController.php`): sprawdza DB (SELECT 1) + cache; zwraca 200/503 JSON; 4 testy integration green; brak dowodu aktywacji Sentry i UptimeRobot | Sentry, uptime, `/health`, alert test i owner incydentów są gotowe | Skonfigurować Sentry DSN w `.env.local` na MyDevil, dodać UptimeRobot monitor na `/health`, wykonać test alertu |
| BETA-BLK-009 | Operations | Runbook release/incydent/support nie istnieje jako jawny artefakt operacyjny | W incydencie zespół będzie improwizował | Tech Writer | READY FOR VERIFY | `docs/RUNBOOK.md` zaktualizowany: 2 delivery paths (main + tag), rollback via tags, contact points (TBD), feedback loop sekcja, Known Gaps tabela | Istnieje runbook z deploy, rollback, restore, incident triage, contact points i został użyty w rehearsal | PO uzupełnia contact points (TBD sekcja), wykonać rehearsal i zaktualizować |
| BETA-BLK-010 | Feedback Loop | Brak jawnej ścieżki zbierania feedbacku z bety i triage P0/P1 | Beta bez feedback pipeline to tylko miękki launch bez kontroli | Product Owner | IN PROGRESS | `docs/BETA_FEEDBACK.md` DRAFT: klasyfikacja P0-P3, rytm weekly review, channel options, exit criteria; czeka na decyzję PO (kanał, owner, SLA) | Jest kanał feedbacku, owner, SLA triage i ścieżka decyzji beta/blocker/backlog | PO decyduje o kanale i właścicielu; wypełnia TBD w BETA_FEEDBACK.md |
