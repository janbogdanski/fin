...
| ~~P2-106~~ | AT-002: Brak tabeli dla snapshot finalizowanych kalkulacji — brak traceability XML↔liczby | Audit Trail S13 | 14 | DONE — tax_calculation_snapshots + TaxCalculationSnapshotRepository, xml_sha256 na każdy eksport |
| ~~P2-107~~ | AT-005: Brak persistent audit log (żadna tabela audit_log/event_store w 10 migracjach) | Audit Trail S13 | 14 | DONE — audit_log (BIGSERIAL, nullable user_id, JSONB context), AuditLogger, user.anonymized event |
...
| P2-117 | TEST P0: AnonymizeUserHandler — brak unit testu; ścieżka "user not found" + atomowość transakcji + idempotentność | QA Coverage | 16 | DONE |
| P2-118 | TEST P1: LossFormValidator — brak unit testu; 6 scenariuszy: comma decimal, boundary 100M, MathException, negative, category null | QA Coverage | 16 | DONE |
| P2-119 | TEST P1: PricingConsentController — brak integration testu; consent missing/invalid plan/valid standard/valid pro | QA Coverage | 16 | DONE |
| P2-120 | TEST P1: UserSubscriptionExtension — brak unit testu; no user/free/standard/pro/pro vs standard priority | QA Coverage | 16 | DONE |
| P2-121 | TEST P1: UserProvider — brak unit testu; supportsClass/loadUserByIdentifier not found/refreshUser wrong type | QA Coverage | 16 | DONE |
| P2-122 | TEST P1: EncryptionKeyInitializer — brak unit testu; main request sets key/sub-request does not | QA Coverage | 16 | DONE |
| P2-123 | TEST P1: GetTaxSummaryAdapter — brak unit testu; deleguje poprawnie do GetTaxSummaryHandler | QA Coverage | 16 | DONE |
| P2-124 | TEST P1: DashboardControllerWebTest — zbyt luźna asercja str_contains(..., '0') — zawęzić | QA Coverage | 16 | DONE |
| P2-125 | TEST P2: MagicLinkAuthenticator — session migrate(true) nie jest explicite testowane | QA Coverage | 16 | DONE |
